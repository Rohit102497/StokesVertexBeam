...
def model_fit(data_dir, batch_size, model_name, num_classes, feature_extract, use_pretrained, lr, momentum, num_epochs):
    ...
    test_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms[x]) for x in ['test_sim']}
    test_dataloaders_dict = {x: torch.utils.data.DataLoader(test_datasets[x], batch_size=1) for x in ['test_sim']}

    # Create experimental datasets
    experimental_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms[x]) for x in ['test_exp']}
    experimental_dataloaders_dict = {x: torch.utils.data.DataLoader(experimental_datasets[x], batch_size=1) for x in ['test_exp']}

    # Accuracy for test data
    test_actual_label = []
    test_prediction = []
    for inputs, labels in test_dataloaders_dict['test_sim']:
    ...
    for inputs, labels in experimental_dataloaders_dict['test_exp']:
